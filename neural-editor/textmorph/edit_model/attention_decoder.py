...
class AttentionContextCombiner(RNNContextCombiner):
    def __call__(self, encoder_output, x):
        return AttentionRNNInput(x=x, agenda=encoder_output.agenda, source_embeds=encoder_output.source_embeds, title_embeds=encoder_output.title_embeds,  insert_embeds=encoder_output.insert_embeds, delete_embeds=encoder_output.delete_embeds)
...
class AttentionDecoderCell(DecoderCell):
    def __init__(self, token_embedder, agenda_dim, decoder_dim, encoder_dim, attn_dim, no_insert_delete_attn, num_layers):
        ...
        augment_dim = encoder_dim + encoder_dim + input_dim + input_dim + agenda_dim
        ...
        z_dim = decoder_dim + 2 * encoder_dim + 2 * input_dim
        ...
        self.title_attention = Attention(encoder_dim, decoder_dim, attn_dim)
    ...
    def initialize(self, batch_size):
        ...
        return AttentionRNNState([h] * self.num_layers, [c] * self.num_layers, init_attn(self.source_attention),
                                 init_attn(self.title_attention),
    ...
    def forward(self, rnn_state, decoder_cell_input, advance):
        ...
        title_attn = self.title_attention(dci.title_embeds, hs[0])
        ...
        if not self.no_insert_delete_attn:
            z = torch.cat([x, source_attn.context, title_attn.context, insert_attn.context, delete_attn.context], 1)
        else:
        ...
        rnn_state = AttentionRNNState(hs, cs, source_attn, title_attn, insert_attn, delete_attn)
...
class AttentionRNNState(namedtuple('AttentionRNNState', ['hs','cs','source_attn', 'title_attn','insert_attn','delete_attn']), RNNState):
    """
    Attributes:
...
class AttentionRNNInput(namedtuple('AttentionRNNInput', ['x','agenda','source_embeds', 'title_embeds', 'insert_embeds','delete_embeds']), RNNInput):
    """
Attributes:
