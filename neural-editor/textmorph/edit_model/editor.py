...
from attention_decoder import AttentionContextCombiner
...
class Editor(Module):
    ...
    def _batch_editor_examples(cls, examples):
        ...
        return source_words, title_words, insert_words, insert_exact_words, delete_words, delete_exact_words, target_words, edit_embed
    ...
    def preprocess(self, examples):
        ...
        Returns:
            EditorInput
        """
        source_words, title_words, insert_words, insert_exact_words, delete_words, delete_exact_words, target_words, edit_embed = self._batch_editor_examples(
            examples)
        encoder_input = self.encoder.preprocess(source_words, title_words, insert_words, insert_exact_words, delete_words,
    ...
    def edit(self, examples, max_seq_length=50, beam_size=5, batch_size=256):
    ...
    def _edit_batch(self, examples, max_seq_length, beam_size):
        source_words, title_words, insert_words, insert_exact_words, delete_words, delete_exact_words, _, edit_embed = self._batch_editor_examples(
            examples)
        # TODO: 对self.encoder.preprocess进行处理
        encoder_input = self.encoder.preprocess(source_words, title_words, insert_words, insert_exact_words, delete_words,
                                                delete_exact_words, edit_embed)
...
class EditExample(namedtuple('EditExample', ['source_words', 'title_words', 'insert_words', 'insert_exact_words', 'delete_words',
    ...
    @classmethod
    def whitelist_blacklist(cls, src_words, trg_words, title_words):
        ...
        return EditExample(source_words=src_words, title_words=title_words, insert_words=[], insert_exact_words=whitelist,
                           delete_words=[], delete_exact_words=blacklist, target_words=trg_words)

    @classmethod
    def salient_diff(cls, src_words, trg_words, free_set, title_words):
        ...
        return EditExample(source_words=src_words,title_words=title_words, insert_words=[], insert_exact_words=insert_words,
                           delete_words=[], delete_exact_words=delete_words, target_words=trg_words)

    def __new__(cls, source_words,title_words, insert_words, insert_exact_words, delete_words, delete_exact_words, target_words,
                edit_embed=None):
        if edit_embed is not None:
            assert len(edit_embed.shape) == 1  # must be 1D array
        self = super(EditExample, cls).__new__(cls, source_words,title_words, insert_words, insert_exact_words, delete_words,
                                               delete_exact_words, target_words, edit_embed)
        return self
