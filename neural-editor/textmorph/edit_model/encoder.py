...
from agenda import AgendaMaker
from edit_encoder import EditEncoder

EncoderInput = namedtuple('EncoderInput',
                          ['source_words', 'title_words', 'insert_words', 'insert_exact_words', 'delete_words', 'delete_exact_words',
                           'edit_embed'])
"""
Args:
...
class EncoderOutput(namedtuple('EncoderOutput', ['source_embeds', 'title_embeds', 'insert_embeds', 'delete_embeds', 'agenda']), NamedTupleLike):
    pass
"""
Args:
...
class Encoder(Module):
    def __init__(self, token_embedder, agenda_dim, edit_dim, hidden_dim, lamb_reg, norm_eps, norm_max, kill_edit, num_layers, rnn_cell_factory):
        ...
        # TODO: 也许可以加入new_encoder?
        self.source_encoder = MultiLayerSourceEncoder(word_dim, hidden_dim, num_layers, rnn_cell_factory)
        self.title_encoder = MultiLayerSourceEncoder(word_dim, hidden_dim, num_layers, rnn_cell_factory)
        self.edit_encoder = EditEncoder(word_dim, edit_dim, lamb_reg, norm_eps, norm_max)
        self.agenda_maker = AgendaMaker(self.source_encoder.hidden_dim, self.title_encoder.hidden_dim, self.edit_dim, self.agenda_dim)

    def preprocess(self, source_words, title_words, insert_words, insert_exact_words, delete_words, delete_exact_words, edit_embed):
        """Preprocess.
        ...
        return EncoderInput(
            ...
            SequenceBatch.from_sequences(title_words, self.word_vocab),
    ...
    def forward(self, encoder_input, draw_samples=False, draw_p = False):
        ...
        title_words = encoder_input.title_words
        title_word_embeds = self.token_embedder.embed_seq_batch(title_words)
        title_encoder_output = self.title_encoder(title_word_embeds.split())
        title_embeds_list = title_encoder_output.combined_states
        title_embeds = SequenceBatch.cat(title_embeds_list)
        title_embeds_final = torch.cat(title_encoder_output.final_states, 1)
        ...
        agenda = self.agenda_maker(source_embeds_final, title_embeds_final, edit_embed)
        return EncoderOutput(source_embeds, title_embeds, insert_noisy_exact, delete_noisy_exact, agenda)
