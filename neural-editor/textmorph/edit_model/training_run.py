...
root_dir = '/var/www/songchuan/comment-generate/prototype_title/neural-editor-data/'
...
class EditDataSplits(object):
    ...
    def __init__(self, data_dir, use_diff):
        ...
        self.train = examples_from_file(join(data_dir, 'train_title_content_30w.tsv'))
        self.valid = examples_from_file(join(data_dir, 'valid_title_content.tsv'))
        self.test = examples_from_file(join(data_dir, 'test_title_content_sub.tsv'))
        self.free = free
...
class EditTrainingRun(TorchTrainingRun):
    ...
    def _evaluate(cls, config, editor, examples, metadata, tb_logger, train_steps, noiser, big_eval, log=True):
        ...
        def evaluate_on_examples(name, examples):
            ...
            edit_traces = cls._compute_metrics(editor, examples, num_eval, noiser,
                                                               edit_dropout=config.editor.edit_dropout,
                                                               draw_samples=config.editor.enable_vae)
    ...
    def _compute_metrics(cls, editor, examples, num_evaluate_examples, noiser,
                         batch_size=32, edit_dropout=False, draw_samples=False):
        with random_seed(0):
