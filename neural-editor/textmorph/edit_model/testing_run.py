...
import data
from attention_decoder import AttentionDecoderCell
from gtd.ml.torch.simple_decoder_cell import SimpleDecoderCell
from edit_noiser import EditNoiser
from editor import Editor, EditExample
        ...
        def examples_from_file(path):
            ...
            with codecs.open(path, 'r', encoding='utf-8') as f:
                for line in verboserate(f, desc='Reading data file.', total=total_lines):
                    try:
                        src, trg, title = line.strip().lower().split('\t')
                        src_words = src.split(' ')
                        trg_words = trg.split(' ')
                        title_words = title.split(' ')
                        assert len(src_words) > 0
                        assert len(trg_words) > 0
                        assert len(title_words) > 0
                    except:
                        pass

                    if use_diff:
                        ex = EditExample.salient_diff(src_words, trg_words, free_set, title_words)
                    else:
                        ex = EditExample.whitelist_blacklist(src_words, trg_words, title_words)
                    examples.append(ex)
            return examples

        self.train = examples_from_file(join(data_dir, 'valid_test3_add_title_content.tsv'))
        self.valid = examples_from_file(join(data_dir, 'valid_test3_add_title_content.tsv'))
        self.test = examples_from_file(join(data_dir, 'valid_test3_add_title_content.tsv'))
        self.free = free
...
class EditTrainingRun(TorchTrainingRun):
        ...
        with random_state(train_state.random_state):
            ...
            while True:
                ...
                for batch in verboserate(train_batches, desc='Streaming training examples'):
                    ...
                    if train_state.train_steps % 1 ==0:
                        results = []
                        for count_test in range(len(examples.valid)/10):
                            num_test = count_test * 10
                            print '==========%d/%d==========' % (count_test, len(examples.valid)/10)
                            cls._evaluate(config, editor, examples, metadata, tb_logger, train_state.train_steps, noiser, num_test, results, big_eval=False)
                            tb_logger.log_value('grad_norm', grad_norm, train_state.train_steps)
                            # if count_test == 2500:
                            #     with open('result_temp.txt', 'w') as f:
                            #         for item in results:
                            #             f.write(item+'\n')
                        with open('result_add_title_content_6.5k.txt', 'w') as f:
                            for item in results:
                                f.write(item+'\n')
                        return
    ...
    @classmethod
    def _evaluate(cls, config, editor, examples, metadata, tb_logger, train_steps, noiser, num_test, results, big_eval, log=True):
    # def _evaluate(cls, config, editor, examples, metadata, tb_logger, train_steps, noiser, big_eval, log=True):
        ...
        def evaluate_on_examples(name, examples):
            ...
            # compute metrics
            loss, avg_bleu, edit_traces, outputs = cls._compute_metrics(editor, examples, num_eval, noiser, num_test,
                                                               edit_dropout=config.editor.edit_dropout,
                                                               draw_samples=config.editor.enable_vae)
            # loss, avg_bleu, edit_traces, outputs = cls._compute_metrics(editor, examples, num_eval, noiser,
            #                                                    edit_dropout=config.editor.edit_dropout,
            #                                                    draw_samples=config.editor.enable_vae)
            ...
            for i in range(len(outputs)):
                result = ' '.join(item.encode('utf-8') for item in outputs[i][0])
                # print result
    ...
    @classmethod
    def _compute_metrics(cls, editor, examples, num_evaluate_examples, noiser, num_test,
                         batch_size=1, edit_dropout=False, draw_samples=False):
    ...
    #                      batch_size=256, edit_dropout=False, draw_samples=False):
        ...
        return loss, avg_bleu, edit_traces, outputs
