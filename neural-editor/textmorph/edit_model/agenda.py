...
class AgendaMaker(Module):
    def __init__(self, source_dim, news_dim, edit_dim, agenda_dim):
        super(AgendaMaker, self).__init__()
        self.linear = Linear(source_dim + news_dim + edit_dim, agenda_dim)

    def forward(self, source_embed, news_embed, edit_embed):
        ...
        """
        inp = torch.cat([source_embed, news_embed, edit_embed], 1)  # (batch_size, hidden_dim + edit_dim)
