...
def word2vec():
    ...
    with open('comment_data.txt') as f:
        tmp = f.readlines()
    data = []
    for i in range(len(tmp)):
        data.append(tmp[i].strip().split('\t')[1])
    ...
    model = gensim.models.Word2Vec(train_data, size=128, min_count=3)
    model.wv.save_word2vec_format('comment_vector.bin', binary=True)
